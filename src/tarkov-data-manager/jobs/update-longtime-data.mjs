...
class UpdateLangJob extends DataJob {
    ...
    run = async () => {
        for(const map in keys){
            ...
            const fileHandle = fs.createWriteStream(path.join(import.meta.dirname, '..', 'public', 'data', `historical-prices-${map}.csv`), {
                flags: 'a',
            });
        ...
        const fileHandle = fs.createWriteStream(path.join(import.meta.dirname, '..', 'public', 'data', `historical-prices-all.csv`), {
            flags: 'a',
        });
